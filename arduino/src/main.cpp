...
bool extractJsonString(const char* line, const char* key, char* out, size_t out_size) {
    ...
    if (value_pos == nullptr || *value_pos != '"') {
        return false;
    }
...
void handleTargetCommand(const char* line) {
    ...
    if (!extractJsonFloat(line, "\"x\"", &x) || !extractJsonFloat(line, "\"y\"", &y) ||
...
void handlePoseCorrection(const char* line) {
    ...
    if (!extractJsonFloat(line, "\"x\"", &x) || !extractJsonFloat(line, "\"y\"", &y) ||
        !extractJsonFloat(line, "\"yaw\"", &yaw) || !extractJsonFloat(line, "\"confidence\"", &confidence)) {
        return;
    }

    const float clamped_conf = clampf(confidence, 0.0f, 1.0f);
    if (clamped_conf <= 0.0f) {
        return;
    }
...
void dispatchLine(const char* line) {
    char type[24];
    if (!extractJsonString(line, "\"type\"", type, sizeof(type))) {
        return;
    }
...
void updateNavigator() {
    ...
    if (g_last_xy_err <= NAV_TARGET_XY_TOL_M && fabsf(g_last_yaw_err) <= NAV_TARGET_YAW_TOL_RAD) {
