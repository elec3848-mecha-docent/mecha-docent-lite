...
class PoseEstimator:
    ...
    def estimate_single_pose(
        ...
        # ---- Reprojection error (D3: outlier rejection + confidence) ----
        # Project the 3D tag corners back through the estimated pose and measure
        # pixel-space residuals.  Tags with large residuals are rejected as outliers.
        _MAX_REPROJ_PX = 15.0  # reject tags with mean corner error above this
        ...
        reproj_error = float(np.mean(
            np.linalg.norm(
                projected_pts.reshape(4, 2) - detection.corners.reshape(4, 2),
                axis=1,
            )
        ))
        if reproj_error > _MAX_REPROJ_PX:
            logger.debug(f"Tag {tag_id} rejected: reproj_error={reproj_error:.1f}px > {_MAX_REPROJ_PX}px")
            return None

        # Normalised confidence [0, 1] from detection quality and reprojection quality.
        # decision_margin ~150 → detect_conf=1.0; ~0 → 0.0
        # reproj_error  0px → reproj_conf=1.0; 15px → 0.0
        detect_conf = min(1.0, float(detection.confidence) / 150.0)
        reproj_conf = max(0.0, 1.0 - reproj_error / _MAX_REPROJ_PX)
        confidence = 0.5 * detect_conf + 0.5 * reproj_conf
