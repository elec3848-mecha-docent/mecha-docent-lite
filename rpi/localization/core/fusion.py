...
def fuse_poses(poses: List[PoseEstimate]) -> PoseEstimate:
    ...
    # Reject outliers: drop poses with confidence below 5% before fusing.
    _MIN_FUSE_CONF = 0.05
    valid = [p for p in poses if p.confidence >= _MIN_FUSE_CONF]
    if not valid:
        valid = poses  # all below threshold: use everything rather than fail
    poses = valid
