...
Four shapes on a BG_COLOR background:
  eye_left, eye_right  — rounded squares of EYE_COLOR
  lid_left, lid_right  — plain squares of BG_COLOR (eyelids)
...
Animations use cubic ease-in-out (smoothstep). Trigger them with:
  ...
  screen.move_lid_left(...)
  screen.move_lid_right(...)
  screen.move_all(eye_l_pos, eye_r_pos, lid_l_pos, lid_r_pos, duration)
...
BG_COLOR: tuple  = (40,  40,  40)   # BGR background + eyelid fill
EYE_COLOR: tuple = (200, 200, 200)  # BGR eye fill

EYE_SIZE:      int = 140   # side length of each eye rounded square (px)
LID_SIZE:      int = 150   # side length of each eyelid square (px)
CORNER_RADIUS: int = 35    # corner radius of the rounded eye squares
...
def _make_rounded_square(size: int, corner_radius: int, color: tuple) -> np.ndarray:
    """Return a BGRA ndarray with a filled, anti-aliased rounded square."""
    canvas = np.zeros((size, size, 4), dtype=np.uint8)
    r = min(corner_radius, size // 2)
    bgra = (int(color[0]), int(color[1]), int(color[2]), 255)
    cv2.rectangle(canvas, (r, 0),      (size - r, size),     bgra, -1)
    cv2.rectangle(canvas, (0, r),      (size,     size - r), bgra, -1)
    for cx, cy in [(r, r), (size - r, r), (r, size - r), (size - r, size - r)]:
        cv2.circle(canvas, (cx, cy), r, bgra, -1)
    return canvas


def _make_square(size: int, color: tuple) -> np.ndarray:
    """Return a BGRA ndarray with a plain filled square."""
    canvas = np.zeros((size, size, 4), dtype=np.uint8)
...
class Screen:
    ...
    Shapes (set starting positions before calling run()):
        eye_left, eye_right  — rounded squares of EYE_COLOR
        lid_left, lid_right  — plain squares of BG_COLOR
    ...
    Example setup:
        ...
        screen.lid_left.snap_to(760, 390)   # above the left eye = open
        screen.lid_right.snap_to(1160, 390)
        screen.run()
    """
    ...
    def __init__(self, width: int = 0, height: int = 0, fps: int = FPS):
        ...
        self.lid_left  = Shape()
        self.lid_right = Shape()

        # Pre-rendered canvases (rebuilt if EYE_SIZE / LID_SIZE change)
        self._eye_canvas = _make_rounded_square(EYE_SIZE, CORNER_RADIUS, EYE_COLOR)
        self._lid_canvas = _make_square(LID_SIZE, BG_COLOR)

    def rebuild_canvases(self) -> None:
        """Call this if you change EYE_SIZE, LID_SIZE, CORNER_RADIUS, or
        the color constants at runtime before run()."""
        self._eye_canvas = _make_rounded_square(EYE_SIZE, CORNER_RADIUS, EYE_COLOR)
        self._lid_canvas = _make_square(LID_SIZE, BG_COLOR)
    ...
    def move_lid_left(
        ...
        self.lid_left.animate_to(x, y, rotation, duration)

    def move_lid_right(
        self, x: float, y: float, rotation: float = 0.0, duration: float = 0.5
    ) -> None:
        self.lid_right.animate_to(x, y, rotation, duration)
    ...
    def move_all(
        ...
        _apply(self.lid_left,  lid_l_pos)
        _apply(self.lid_right, lid_r_pos)
    ...
    def _update(self) -> None:
        for shape in (self.eye_left, self.eye_right, self.lid_left, self.lid_right):
            shape.update()
    ...
    def _render(self, frame: np.ndarray) -> None:
        ...
        _composite(frame, self._eye_canvas,
                   int(self.eye_left.x),  int(self.eye_left.y),  self.eye_left.rotation)
        _composite(frame, self._eye_canvas,
                   int(self.eye_right.x), int(self.eye_right.y), self.eye_right.rotation)
        _composite(frame, self._lid_canvas,
                   int(self.lid_left.x),  int(self.lid_left.y),  self.lid_left.rotation)
        _composite(frame, self._lid_canvas,
                   int(self.lid_right.x), int(self.lid_right.y), self.lid_right.rotation)
